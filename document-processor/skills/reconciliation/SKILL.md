...
|----------|---------|-------------|
| `--workspace` | `workspace` | Workspace directory containing expenses.csv and parsed_invoices.json |
...
2. Loads `parsed_invoices.json` from `{workspace}/` (generated by the `pdf-parsing` skill).
...
5. Generates `{workspace}/reconciliation_report.md` with summary stats and discrepancy tables.
6. Generates `{workspace}/reconciliation_data.json` with machine-readable matched results.
